...
@section('title', trans('admin.lesson.actions.index'))

@section('body')

    <lesson-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/lessons') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.lesson.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/lessons/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.lesson.columns.id') }}</th>
                                    <th is='sortable' :column="'title'">{{ trans('admin.lesson.columns.title') }}</th>
                                    <th is='sortable'
                                        :column="'course_section_id'">{{ trans('admin.lesson.columns.course_section_id') }}</th>
                                    <th is='sortable'
                                        :column="'duration_minute'">{{ trans('admin.lesson.columns.duration_minute') }}</th>
                                    <th is='sortable' :column="'level'">{{ trans('admin.lesson.columns.level') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/lessons')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/lessons/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tr v-for="(item, index) in collection" :key="item.id"
                                    ...
                                    <td>@{{ item.id }}</td>
                                    <td>@{{ item.title }}</td>
                                    <td>@{{ item.course_section ? item.course_section.title : '' }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/lessons/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.lesson.actions.create') }}</a>
                            </div>
                        </div>
